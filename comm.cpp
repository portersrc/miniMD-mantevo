...
Comm::Comm()
{
  ...
  d_buf_send = (MMD_float*) acc_malloc((maxsend + BUFMIN) * sizeof(MMD_float));
  maxrecv = BUFMIN;
  buf_recv = (MMD_float*) malloc(maxrecv * sizeof(MMD_float));
  d_buf_recv = (MMD_float*) acc_malloc(maxrecv * sizeof(MMD_float));
...
void Comm::communicate(Atom &atom)
{
  ...
  for(iswap = 0; iswap < nswap; iswap++) {
    ...
    //
    //printf("C1\n");
    atom.pack_comm(sendnum[iswap], sendlist[iswap], d_buf_send, pbc_flags);
    //printf("C2\n");
    ...
    if(sendproc[iswap] != me) {
      
      {
        if(sizeof(MMD_float) == 4) {
          MPI_Irecv(d_buf_recv, comm_recv_size[iswap], MPI_FLOAT,
          recvproc[iswap], 0, MPI_COMM_WORLD, &request);
          MPI_Send(d_buf_send, comm_send_size[iswap], MPI_FLOAT,
          sendproc[iswap], 0, MPI_COMM_WORLD);
        } else {
          MPI_Irecv(d_buf_recv, comm_recv_size[iswap], MPI_DOUBLE,
          recvproc[iswap], 0, MPI_COMM_WORLD, &request);
          MPI_Send(d_buf_send, comm_send_size[iswap], MPI_DOUBLE,
          sendproc[iswap], 0, MPI_COMM_WORLD);
        }
...
void Comm::growsend(int n)
{
  ...
  acc_free(d_buf_send);
  d_buf_send = (MMD_float*) acc_malloc((maxsend+BUFEXTRA)*sizeof(MMD_float));
}
