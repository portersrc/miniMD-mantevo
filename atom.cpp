...
void Atom::sync_host(void* h_ptr_, void* d_ptr_, int bytes) {
...
void Atom::sync_device(void* d_ptr_, void* h_ptr_, int bytes) {
...
void Atom::growarray()
{
  ...
  acc_free(d_v);
  acc_free(d_f);
  d_x = (MMD_float*) acc_malloc(nmax*PAD*sizeof(MMD_float));
  d_v = (MMD_float*) acc_malloc(nmax*PAD*sizeof(MMD_float));
  d_f = (MMD_float*) acc_malloc(nmax*PAD*sizeof(MMD_float));
...
void Atom::pack_comm(int n_, int* list_, MMD_float* buf_, int* pbc_flags)
{
  const int n = n_;
  const int* const restrict list = list_;
  MMD_float* const restrict buf = buf_;
  const MMD_float* const restrict x_ = d_x;
...
void Atom::unpack_comm(int n_, int first_, MMD_float* buf_)
{
  const int n = 3*n_;
  const int first = first_*PAD;
  MMD_float* const restrict x_ = d_x;
